...
FileManager::FileManager() {
	...
	f = File(PATH);
	file = &f;
	usersMap = map<string,pair<string,int> >();
	fileIn.open(PATH);
}
...
//	cout<<"Read line: "<<fullLineStr<<endl;
...
//		cout<<"User string: "<<userString<<endl;
...
//	cout<<"writeUser's prev line: "<<fullLine<<endl;
...
//void FileManager::writeUser(User user, map<string, pair<string, int> >* usersMap) {
//
//	Guard guard(&mutex);
//	char coma = ',';
//
...
//	file->write((char *)&coma,1);
//	file->write(name.data(),name.length());
//	file->write((char *)&coma,1);
//
//	string pass = user.getPass();
//	int passLen = pass.length();
//	file->write((char *)&passLen,4);
//	file->write((char *)&coma,1);
//	file->write(pass.data(),pass.length());
//	file->write((char *)&coma,1);
//
//	int score = user.getScore();
...
//	file->write((char *)"\n",1);
//
//	//Finished writing the user into the file, insert it to the map!
//	usersMap->insert(make_pair(name,make_pair(pass,score)));
//}
...
void FileManager::setUsersMap(const map<string, pair<string, int> >& usersMap) {
